...
mapwork.model.Map = function () {
    ...
    this.createBlankModel = function (name, tileWidth, tileHeight, tilesAccross, tilesDown) {
        var newLayer;
        ...
        newLayer.createBlankModelLayer(this, ('Untitled Layer'), 'default_tileset.png');
    ...
    this.createModelFromJSONString = function (json) {
        ...
        // property assignment 
        for (propertyCount = 0; propertyCount < json.properties.length; propertyCount++) {
    ...
    this.serialize = function () {
        ...
        var obj, layerCount, layer, rowCount, cellCount, allLayers, row, currentRow, cell, jsonString;
    ...
    this.resizeMap = function (specifications) {
        var layerCount, rowCount, layers, cellCount, currentLayer, currentRow, tileDifference, newRow, newTile;
        ...
        if (specifications.tilesDown) {
            tileDifference = (Math.abs(this.getTilesDown() - parseInt(specifications.tilesDown, 10)));
            ...
            if (this.getTilesDown() > parseInt(specifications.tilesDown, 10)) {
                ...
                for (layerCount = 0; layerCount < layers.length; layerCount++) {
                    ...
                    currentLayer.getRows().splice((parseInt(specifications.tilesDown, 10)), tileDifference);
                }
            }
            else {
                // expansion 
                for (layerCount = 0; layerCount < layers.length; layerCount++) {
                    for (rowCount = 0; rowCount < tileDifference; rowCount++) {
                        currentLayer = layers[layerCount];
                        newRow = [];
                        for (cellCount = 0; cellCount < this.getTilesAccross() ; cellCount++) {
        ...
        if (specifications.tilesAccross) {
            tileDifference = (Math.abs(this.getTilesAccross() - parseInt(specifications.tilesAccross, 10)));
            ...
            if (this.getTilesAccross() > parseInt(specifications.tilesAccross, 10)) {
                ...
                for (layerCount = 0; layerCount < layers.length; layerCount++) {
                    currentLayer = layers[layerCount];
                    for (rowCount = 0; rowCount < currentLayer.getRows().length; rowCount++) {
                        currentRow = currentLayer.getRow(rowCount);
                        currentRow.splice((parseInt(specifications.tilesAccross, 10)), tileDifference);
                    }
                }
            }
            else {
