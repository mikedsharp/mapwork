...
mapwork.model.Layer = function () {
    ...
    this.setTilesetImage = function (path) {
        ...
        $(this.tilesetImage).on('load', $.proxy(function () {
    ...
    this.createModelLayerFromJSONObject = function (map, json) {
        ...
        for (layerPropertyCount = 0; layerPropertyCount < json.properties.length; layerPropertyCount++) {
            this.addProperty(json.properties[layerPropertyCount]);
        }

        for (rowCount = 0; rowCount < json.rows.length; rowCount++) {
            row = [];
            for (cellCount = 0; cellCount < json.rows[rowCount].cells.length; cellCount++) {
                cell = new mapwork.model.Tile();
                cell.setTileCode(json.rows[rowCount].cells[cellCount].tileCode);

                for (propertyCount = 0; propertyCount < json.rows[rowCount].cells[cellCount].properties.length; propertyCount++) {
                    cell.addProperty(json.rows[rowCount].cells[cellCount].properties[propertyCount]);
                }
